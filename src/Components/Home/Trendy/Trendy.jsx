...
const Trendy = () => {
  ...
  const sortByWeightedScore = (a, b) => {
    // Convert review counts (e.g., "8k+ reviews" to 8000)
    const getReviewCount = (reviews) => {
      const count = parseInt(reviews.replace("k+ reviews", "").replace(",", ""));
      return reviews.includes("k") ? count * 1000 : count;
    };

    // Calculate weighted score (70% rating, 30% normalized review count)
    const calculateScore = (product) => {
      const reviewCount = getReviewCount(product.productReviews);
      const maxReviews = 9000; // 9k is our highest review count
      const normalizedReviews = reviewCount / maxReviews;
      
      return (product.rating * 0.7) + (normalizedReviews * 0.3);
    };
